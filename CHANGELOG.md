...
- sql-reviewer:扫描 SQL 反模式,高危操作(缺 WHERE 的 UPDATE/DELETE、DROP/TRUNCATE)必须处理,性能类建议先看执行计划再改

## v0.10.0(2026-08-30)

技能数量:25,其中 23 个带可执行脚本,22 个带参考资料。

### 新增技能

- meeting-minutes:把会议讨论整理成含结论与待办的中文纪要,待办必须有负责人和截止时间,附纪要检查脚本

### 工程

- 技能风格与文档检查接入 CI 质量门禁,检查器支持 strict 模式

## v0.9.0(2026-08-30)

技能数量:24,其中 22 个带可执行脚本,22 个带参考资料。

### 新增技能

- release-note-writer:从 git 历史生成发版说明或 CHANGELOG 条目,破坏性变更单独列出
...
技能数量:23,其中 21 个带可执行脚本,22 个带参考资料。

### 新增技能

- weekly-report:收集本周 git 提交与工作事项,生成"完成/进行中/风险/下一步"结构的中文周报,附提交收集脚本
